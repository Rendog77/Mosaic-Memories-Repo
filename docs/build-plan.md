...
**Status:** In progress. The platform-independent Swift package foundation, versioned persistence and migrations, workflow state, privacy-validated coarse analytics, tests, macOS CI, SwiftUI creation shell, root navigation, adaptive design tokens, autosave/restoration, saved-project catalogue and management, recovery reporting, dependency-injected photo selection, and typed source-set validation are scaffolded. The Xcode application target, real PhotoPicker controls, and Apple simulator/device validation remain pending.
